...
- [ ] **Live write creates/refreshes a real playlist — BLOCKED by Spotify policy (not code).** *Verified live on 2026-06-21: ingest → score → URI resolution all work against the real account; the dry-run resolves 15 real Spotify URIs. The write returns 403 for both create and modify despite Premium + allowlist + scopes + fresh consent — Spotify gates all programmatic playlist writes behind **Extended Quota Mode** (app review). The write path is implemented + mock-tested (`test_spotify_writer_creates_then_replaces` / `test_spotify_writer_refreshes_existing`); it cannot be box-checked until quota is granted. See DECISIONS D14.*
